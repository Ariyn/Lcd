...
func testUserToManyOwnerUIDArticles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.OwnerUIDArticles().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.OwnerUID == b.OwnerUID {
			bFound = true
		}
		if v.OwnerUID == c.OwnerUID {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadOwnerUIDArticles(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerUIDArticles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.OwnerUIDArticles = nil
	if err = a.L.LoadOwnerUIDArticles(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerUIDArticles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpOwnerUIDArticles(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddOwnerUIDArticles(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.UID != first.OwnerUID {
			t.Error("foreign key was wrong value", a.UID, first.OwnerUID)
		}
		if a.UID != second.OwnerUID {
			t.Error("foreign key was wrong value", a.UID, second.OwnerUID)
		}

		if first.R.OwnerUIDUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.OwnerUIDUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.OwnerUIDArticles[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.OwnerUIDArticles[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.OwnerUIDArticles().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`UID`: `int`, `ID`: `char`, `Name`: `char`}
	_           = bytes.MinRead
)
