...
var UserRels = struct {
	OwnerUIDArticles string
}{
	OwnerUIDArticles: "OwnerUIDArticles",
}

// userR is where relationships are stored.
type userR struct {
	OwnerUIDArticles ArticleSlice `boil:"OwnerUIDArticles" json:"OwnerUIDArticles" toml:"OwnerUIDArticles" yaml:"OwnerUIDArticles"`
}
...
var (
	userAllColumns            = []string{"uid", "id", "name"}
	userColumnsWithoutDefault = []string{"id", "name"}
	userColumnsWithDefault    = []string{"uid"}
...
// OwnerUIDArticles retrieves all the article's Articles with an executor via owner_uid column.
func (o *User) OwnerUIDArticles(mods ...qm.QueryMod) articleQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`article`.`owner_uid`=?", o.UID),
	)
...
// LoadOwnerUIDArticles allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadOwnerUIDArticles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`article`),
		qm.WhereIn(`article.owner_uid in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.OwnerUIDArticles = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &articleR{}
			}
			foreign.R.OwnerUIDUser = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UID == foreign.OwnerUID {
				local.R.OwnerUIDArticles = append(local.R.OwnerUIDArticles, foreign)
				if foreign.R == nil {
					foreign.R = &articleR{}
				}
				foreign.R.OwnerUIDUser = local
				break
			}
		}
	}
...
// AddOwnerUIDArticlesG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.OwnerUIDArticles.
// Sets related.R.OwnerUIDUser appropriately.
// Uses the global database handle.
func (o *User) AddOwnerUIDArticlesG(ctx context.Context, insert bool, related ...*Article) error {
	return o.AddOwnerUIDArticles(ctx, boil.GetContextDB(), insert, related...)
}

// AddOwnerUIDArticles adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.OwnerUIDArticles.
// Sets related.R.OwnerUIDUser appropriately.
func (o *User) AddOwnerUIDArticles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Article) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			OwnerUIDArticles: related,
		}
	} else {
		o.R.OwnerUIDArticles = append(o.R.OwnerUIDArticles, related...)
	}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, uID int, selectCols ...string) (*User, error) {
	...
	query := fmt.Sprintf(
		"select %s from `user` where `uid`=?", sel,
	)
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, uID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `user` where `uid`=? limit 1)"
